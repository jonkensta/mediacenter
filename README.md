...
sudo mkdir -p /opt/mediaserver/{radarr,sonarr,lidarr,bazarr,prowlarr,navidrome,deluge,heimdall,jellyfin,endlessh,gluetun,foundryvtt,cloudflare-ddns,swag}
...
2. **Enable proxy configurations for each service**:
   ...
   sudo cp navidrome.subdomain.conf.sample navidrome.subdomain.conf
